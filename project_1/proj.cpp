// Intro to Computer Graphics - Project 1 - Variable Sphere
...
const char *WINDOWTITLE = {"Variable Sphere (P1) -- Nicholas Olson"};
const char *GLUITITLE = {"Variable Sphere UI"};
...
const float ROTATE_Z_SPEED = 0.f;
const float ROTATE_Y_SPEED = 0.12f;
const float ROTATE_X_SPEED = 0.07f;
...
void Animate()
{
    ...
    // float wow = ((float)elapsedMS) / ROTATE_TIME;
    // printf("ms=%i; elapsed=%i; xdiff=%.3f; \n", ms, elapsedMS, wow);
    // printf("Zrot=%.3f; Yrot=%.3f; Xrot=%.3f; \n\n", Zrot, Yrot, Xrot);

    // Handle rotation if enabled
    if (RotateOn) {
        Zrot += 360.f * ROTATE_Z_SPEED * ((float)elapsedMS) / ROTATE_TIME;
        Yrot += 360.f * ROTATE_Y_SPEED * ((float)elapsedMS) / ROTATE_TIME;
        Xrot += 360.f * ROTATE_X_SPEED * ((float)elapsedMS) / ROTATE_TIME;
        // Zrot += (float)(ms % (int)(ROTATE_TIME / ROTATE_Z_SPEED)) / (ROTATE_TIME / ROTATE_Z_SPEED);
        // Zrot += (float)(ms % (int)(ROTATE_TIME / ROTATE_Y_SPEED)) / (ROTATE_TIME / ROTATE_Y_SPEED);
        // Zrot += (float)(ms % (int)(ROTATE_TIME / ROTATE_X_SPEED)) / (ROTATE_TIME / ROTATE_X_SPEED);
        // Yrot = ROTATE_Y_SPEED * rotateTime;
        // Xrot = ROTATE_X_SPEED * rotateTime;
    }
...
void Display()
{
    ...
    sprintf(sliceStr,
            ...
            SphereVertSlices, SphereArcSlices);
    ...
    for (int i = 0; i <= SphereVertSlices; i++) {
        ...
        for (int j = 0; j <= SphereArcSlices; j++) {
            ...
            if (!first) {
                ...
                glColor3f(float(i) / SphereVertSlices,
                          0.f,
                          float(j) / SphereArcSlices);
