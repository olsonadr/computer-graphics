...
struct GLbinarytype BinaryTypes[] =
    {
...
GLSLProgram::GLSLProgram(int n, int *attrib_idx, const char **attrib_names)
{
    this->attrib_n = n;

    this->attrib_idx = (int*) malloc(n*sizeof(int));
    std::copy(attrib_idx, &attrib_idx[n], this->attrib_idx);

    this->attrib_names = (char**) malloc(n*sizeof(char*));
    for (int i = 0; i < n; i++) {
        this->attrib_names[i] = (char*) malloc(sizeof(char)*(std::strlen(attrib_names[i])+1));
        std::strcpy(this->attrib_names[i], attrib_names[i]);
    }

    CommonConstructor();
}

void GLSLProgram::CommonConstructor() {
    Verbose = false;
...
{
    if (this->attrib_n > 0) {
        free(this->attrib_idx);
        for (int i = 0; i < this->attrib_n; i++)
            free(this->attrib_names[i]);
        free(this->attrib_names);
    }
}
...
bool GLSLProgram::CreateHelper(char *file0, ...)
{
    ...
    for (int i = 0; i < attrib_n; i++)
        glBindAttribLocation(Program, attrib_idx[i], attrib_names[i]);
...
void GLSLProgram::DispatchCompute(GLuint num_groups_x, GLuint num_groups_y, GLuint num_groups_z)
{
    Use();
...
#ifdef VEC3_H
void GLSLProgram::SetAttributeVariable(char *name, Vec3 &v);
{
    int loc;
    if ((loc = GetAttributeLocation(name)) >= 0)
    {
...
void GLSLProgram::SetAttributeVariable(char *name, VertexBufferObject &vb, GLenum which)
{
    int loc;
    if ((loc = GetAttributeLocation(name)) >= 0)
    {
    ...
    void
    GLSLProgram::SetUniformVariable(const char *name, float val0, float val1, float val2, float val3)
    {
        int loc;
        if ((loc = GetUniformLocation(name)) >= 0)
        {
    ...
    void
    GLSLProgram::SetUniformVariable(const char *name, glm::vec3 vals)
    {
        int loc;
        // fprintf(stderr, "Found a 3-element glm array\n");

        if ((loc = GetUniformLocation(name)) >= 0)
        {
            this->Use();
            // glUniform3fv(loc, 3, &vals[0]);
            glUniform3f(loc, vals[0], vals[1], vals[2]);
        }
    };
    ...
    #include<iostream>
    void
    GLSLProgram::SetUniformVariable(const char *name, glm::vec4 vals)
    {
        int loc;
        // fprintf(stderr, "Found a 4-element glm array\n");

        if ((loc = GetUniformLocation(name)) >= 0)
        {
            this->Use();
            // glUniform4fv(loc, 4, &vals[0]);
            glUniform4f(loc, vals[0], vals[1], vals[2], vals[3]);
        }
    };

    void
    GLSLProgram::SetUniformVariable4(const char *name, float vals[4])
    {
        int loc;
        // fprintf(stderr, "Found a 4-element array\n");

        if ((loc = GetUniformLocation(name)) >= 0)
        {
            this->Use();
            // glUniform4fv(loc, 4, vals);
            glUniform4f(loc, vals[0], vals[1], vals[2], vals[3]);
        }
    };

    void
    GLSLProgram::SetUniformVariableMat4(const char *name, float vals[16])
    {
        int loc;
        // fprintf(stderr, "Found a mat4 array\n");

        if ((loc = GetUniformLocation(name)) >= 0)
        {
            this->Use();
            glUniformMatrix4fv(loc, 1, false, vals);
        }
    };
...
#ifdef VEC3_H
    void
    GLSLProgram::SetUniformVariable(const char *name, Vec3 &v);
    {
        int loc;
        if ((loc = GetAttributeLocation(name)) >= 0)
        {
...
#ifdef MATRIX4_H
    void
    GLSLProgram::SetUniformVariable(const char *name, Matrix4 &m)
    {
        int loc;
        if ((loc = GetUniformLocation(name)) >= 0)
        {
...
#define CHECK_GL_ERRORS
    ...
    void GLSLProgram::SetGstap(bool b)
    {
        IncludeGstap = b;
    }
